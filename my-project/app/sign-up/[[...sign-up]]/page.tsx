...
import { Loader2, Star, Moon, Sun, Mail, Lock, User } from "lucide-react"
...
export default function CosmicSignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        router.push("/cosmicDashboard")
      } else {
        console.error("Sign up failed", result)
        // Updated error message handling
        setError("Sign up failed: " + (result as any).errors?.[0]?.message || "Please try again.")
      }
    } catch (err) {
      console.error("Error during sign up", err)
      // Updated error message for clarity
      setError("Error during sign up: " + ((err as Error).message || "A celestial disturbance occurred. Please try again."))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      // Updated error message for clarity
      setError("Google sign-up failed: " + ((err as Error).message || "A cosmic anomaly prevented Google sign-up. Please try again."))
    }
  }
      ...
      <Card className="w-full max-w-md mx-auto bg-opacity-30 bg-black backdrop-blur-md border-purple-500 text-purple-100 shadow-lg shadow-purple-500/20">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center text-purple-300">
            Embark on your celestial journey and join the astral community
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="username"
                placeholder="Your stellar identity"
                value={username}
          ...
          <div className="mt-4 text-center">
            <a href="/sign-in" className="text-purple-300 hover:text-purple-200 transition-colors">
              Already a cosmic traveler? Sign in to your astral account
            </a>
          </div>
        </CardContent>
      </Card>
